...
@bot.callback_query_handler(
    func=lambda call: call.data in ["pay_dc", "pay_alif"]
)
def payment_method(call):
  ...
  text = (
      ...
      f"Рақами карта:\n`{card_number}`\n\n"
      "✍️ **Қадами 1:** Лутфан **ID бозии худро** (рақами ID-и дар дохили бозӣ"
      " доштаатонро) ба инҷо нависед ва фиристед:"
  )
  bot.edit_message_text(
      text,
      call.message.chat.id,
...
@bot.message_handler(
    func=lambda message: (
        ...
        and user_data[message.from_user.id].get("step") == "waiting_game_id"
    )
)
...
def receive_photo(message):
  ...
  if user_id not in user_data or user_data[user_id].get(
      "step"
  ) != "waiting_photo":
    bot.reply_to(
        message,
        "Лутфан аввал тугмаи «Пополнить баланс»-ро пахш кунед ва усули пардохтро"
        " интихоб намоед.",
    )
    return
  ...
  btn_approve = InlineKeyboardButton(
      "✅ Тасдиқ ва илова ба баланс",
      callback_data=f"approve_{user_id}_{game_id}",
  )
  btn_reject = InlineKeyboardButton(
      "❌ Рад кардан", callback_data=f"reject_{user_id}"
  )
  ...
  bot.reply_to(
      message,
      "✅ Чек ва ID бозии шумо қабул шуд! Администратор чекро санҷида истодааст."
...
@bot.callback_query_handler(
    func=lambda call: (
        call.data.startswith("approve_") or call.data.startswith("reject_")
    )
)
def admin_decision(call):
  if call.from_user.id != ADMIN_ID:
    bot.answer_callback_query(
        call.id, "Шумо ҳуқуқи иҷрои ин амалро надоред!", show_alert=True
    )
    return
  ...
  if action == "approve":
    ...
    bot.send_message(
        ADMIN_ID,
        "✍️ Лутфан миқдори маблағеро (бо сомонӣ), ки бояд ба ID бозии"
        f" **{game_id}** илова шавад, бо рақам нависед (масалан: `50`):",
        parse_mode="Markdown",
    )
  elif action == "reject":
...
@bot.message_handler(
    func=lambda message: (
        message.from_user.id == ADMIN_ID and ADMIN_ID in admin_state
    )
)
def admin_enter_amount(message):
  try:
    ...
    bot.reply_to(
        message, "⚠️ Лутфан танҳо рақам нависед (масалан: 50 ё 100):"
    )
    return
  ...
  bot.reply_to(
      message,
      f"✅ Муваффақият! {amount} сомонӣ ба ID бозии `{game_id}` илова шуд."
      f" Баланси умумии ин ID: {new_balance} сом.",
      parse_mode="Markdown",
  )

  bot.send_message(
      target_user_id,
      "🎉 Чек тасдиқ шуд! Ба миқдори **{amount} сомонӣ** ба баланси бозии шумо"
      f" (ID: `{game_id}`) илова гардид. Баланси нави шумо: **{new_balance}"
      " сомонӣ**.",
      parse_mode="Markdown",
  )

...
# Коди боти шумо
TOKEN = "8942662207:AAHSM8rvX7pBLTCLrXWY40jfFZkEXxgE2yU"
ADMIN_ID = 5108777990
...
def start(message):
  ...
  bot.send_message(
      message.chat.id,
      "Хуш омадед ба Apple Game Casino! Бозӣ кунед ва баъд балансатонро пур"
      " кунед:",
      reply_markup=markup,
  )
...
def deposit(call):
  ...
  btn1 = InlineKeyboardButton(
      "DC (Dushanbe City)", callback_data="pay_dc"
  )
...
def payment_method(call):
  ...
  text = (
      ...
      " аз гузаронидани маблағ, скриншоти чекро ба ҳаминҷо фиристед!"
  )
  bot.edit_message_text(
      text,
      call.message.chat.id,
...
def receive_photo(message):
  ...
  btn_approve = InlineKeyboardButton(
      "✅ Тасдиқ (Пур кардан)", callback_data=f"approve_{user.id}"
  )
  btn_reject = InlineKeyboardButton(
      "❌ Рад кардан", callback_data=f"reject_{user.id}"
  )
  ...
  bot.reply_to(
      message,
      "✅ Чек қабул шуд! Администратор онро тафтиш карда истодааст. Лутфан"
      " интизор шавед.",
  )
...
def admin_decision(call):
  ...
  if action == "approve":
    ...
    bot.send_message(
        target_user_id,
        "❌ Чек рад карда шуд ё нодуруст аст. Маблағ ба баланс ворид"
        " нашуд.",
    )
